...
func set_status(new_status: int) -> void:
	...
	match status:
		Status.PASSED:
			_icon.texture = preload("icons/checkmark_valid.svg")
			modulate = COLOR_PASSED
		Status.FAILED:
			_icon.texture = preload("icons/checkmark_invalid.svg")
			modulate = COLOR_FAILED
		Status.PENDING:
			_icon.texture = preload("icons/checkmark_none.svg")
