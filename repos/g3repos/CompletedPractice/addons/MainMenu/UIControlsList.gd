...
func setup(input_actions: Array, movement_scheme := -1) -> void:
	for action in input_actions:
		add_control_from_input_action(action)
	if movement_scheme != -1:
		add_controls_from_scheme(movement_scheme)

	if not input_actions and movement_scheme == MovementSchemes.NONE:
		toggle_panel(false)
...
func add_controls_from_scheme(scheme: int) -> void:
	var actions: Array = MOVEMENT_KEY_MAPPINGS[scheme]
